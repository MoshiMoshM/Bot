...
if cookies_content:
    ...
    print(f"✅ Cookies заредени от environment variable")

def get_ydl_opts(extract_flat=True):
    opts = {
        "format": "bestaudio/best[ext=webm]/bestaudio/best/worstaudio",
        "quiet": True,
        "no_warnings": True,
        "noplaylist": False,
...
async def get_stream_url(url):
    loop = asyncio.get_event_loop()
    def _get():
        with yt_dlp.YoutubeDL(get_ydl_opts(extract_flat=False)) as ydl:

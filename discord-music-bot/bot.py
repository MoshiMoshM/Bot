...
async def fetch_info(query: str) -> list[dict]:
    """Връща списък с {title, url} за query или YouTube URL."""
...
async def get_stream_url(url: str) -> tuple[str, str]:
    """Връща (stream_url, title) за директно пускане."""
    loop = asyncio.get_event_loop()
...
async def stop(ctx: commands.Context):
    """!stop — спира и изчиства опашката"""
    if ctx.voice_client:
...
if __name__ == "__main__":
    ...
    if not token:
        print("❌ Липсва DISCORD_TOKEN! Задай го в .env файла.")
    else:
        bot.run(token)
